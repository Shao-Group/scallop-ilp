...
int scallop::assemble(GRBEnv *env, map<string, int> choose_map)
{
	int c = classify();
    ...
    /*if(N>0 && hs.edges.size()>0)
    {
        ...
        if(hs.edges.size()<= 10)
            num_ilp_10++;
        if(hs.edges.size()>10 && hs.edges.size()<100 && ave_ewrt >= 25)
        {
            num_ilp_100++;
        }
        if(hs.edges.size()>10 && hs.edges.size()<300 && ave_ewrt >= 25)
        {
            num_ilp_300++;
        }

    }
    printf("Total: %d\t(0,10]:%d\t(10,100):%d\t(10,300):%d\n", num_total, num_ilp_10, num_ilp_100, num_ilp_300);
    return 0;*/
    ...
    /*for(int i = 0; i < gr.num_edges(); i++)
    {
        ...
        cout << "Edge-" << i << endl;
        cout << e->source() << " -> " << e->target() << " Weight: " << gr.get_edge_weight(e) << endl;
    }

    for(int i = 0; i < hs.edges.size(); i++)
    {
        cout << "HS-" << i << ": ";
        for(int j = 0; j < hs.edges[i].size(); j++)
        {
            cout << hs.edges[i][j] << '\t';
        }
        cout << " Weight: " << hs.ecnts[i] << endl;
    ...
    int err = 1;
    /*vector<path> paths_ilp;
    if( N>0 && gr.num_edges()>1 && hs.edges.size()>0 && hs.edges.size()<100)
    {
        ofstream stat_file;
        stat_file.open("stat.csv", fstream::app);
        stat_file << gr.gid.c_str() << ',' << gr.num_vertices() << ',' << gr.num_edges() << ',' <<  hs.edges.size() << ',' << ave_ewrt << ',';
        stat_file.close();
    ...
    MEI e2i_ori;
    VE i2e_ori;
    gr_ori.get_edge_indices(i2e_ori, e2i_ori);
	hyper_set hs_ori;
    hs_ori.build(gr_ori, e2i_ori);*/
    ...
    for(int k = 0; k < paths.size(); k++) paths[k].algo = "scallop";
